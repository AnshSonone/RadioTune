...
export default function Home() {
  ...
  const [backgroundTheme, setBackgroundTheme] = useState('bg-[#F3F5F9] text-[#14161F]')
  ...
  return (
    <div className={` theme-surface theme-page  bg-(--theme-bg) text-(--theme-text) min-h-screen `}>
      {/* Hero */}
      <main className="relative overflow-hidden">
        ...
        <section className="relative z-20 max-w-2xl mx-auto px-6 pt-20 sm:pt-28 pb-16 text-center">
          ...
          <h1 className={`${display.className} text-[2.6rem] sm:text-[3.4rem] leading-[1.08] mb-10`}>
            <span className="text-transparent bg-clip-text bg-linear-to-r from-blue-500 to-purple-600 font-bold">Listen, </span>
          ...
          <div className="relative">
            <form onSubmit={handleSearch} className="group flex items-center gap-3 rounded-full border border-[#DDE1EA] px-5 py-4 shadow-[0_1px_2px_rgba(20,22,31,0.04)] focus-within:border-[#4B3BFF] focus-within:shadow-[0_0_0_4px_rgba(75,59,255,0.12)] transition-shadow theme-surface theme-page  bg-(--theme-bg) text-(--theme-text)">
              <button
                type="submit"
                aria-label="Search"
              ...
              />
              <div className="hidden sm:flex items-end gap-0.75 h-5" aria-hidden="true">
                {[6, 14, 9, 18, 7].map((h, i) => (
                  <span
                    key={i}
              ...
              <div className="absolute left-0 right-0 top-full z-10 mt-2 overflow-hidden rounded-2xl border border-gray-700 theme-surface theme-page  bg-(--theme-bg) text-(--theme-text) py-2 text-left shadow-[0_12px_30px_rgba(20,22,31,0.12)]">
                {suggestions.map((song, index) => (
                  ...
                  >
                    ...
                    <span className="min-w-0 truncate text-sm font-medium text-(--theme-text)">{song.name}</span>
