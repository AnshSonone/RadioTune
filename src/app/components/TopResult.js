    ...
    return (
        <div className="lg:col-span-2">
                      ...
                      <div className="bg-[#] border border-zinc-900 p-6 rounded-2xl relative group hover:bg-zinc-800/40 transition cursor-pointer">
                        <div
                          className={`w-24 h-24 bg-zinc-800 flex items-center justify-center mb-6 relative overflow-hidden ${topResult.type === "Artist" ? "rounded-full shadow-2xl" : "rounded-xl shadow-lg"}`}
                        >
                          {topResult.type === "Artist" ? (
                            <User size={40} className="text-zinc-400" />
                          ) : !topResult?.thumbnails[0]?.url ? (
                            <Music size={40} className="text-zinc-400" />
                          ) : (
                            <Image
                              src={topResult?.thumbnails[0]?.url}
                              fill
                              sizes="100px"
                              alt="Cover"
                        ...
                        <button onClick={songPlaying} className="absolute bottom-6 right-6 w-12 h-12 bg-white text-black rounded-full flex items-center justify-center  shadow-xl">
